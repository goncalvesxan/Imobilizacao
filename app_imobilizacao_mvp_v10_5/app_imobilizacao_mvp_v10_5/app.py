...
.hover-scroll {max-height: 280px; overflow:auto; border:1px solid #E2E8F0; border-radius:14px; background:#FFFFFF; box-shadow:0 4px 16px rgba(15,23,42,.045);}
.hover-table {border-collapse:collapse; width:100%; min-width:960px; font-size:.82rem;}
.hover-table th {position:sticky; top:0; background:#F8FAFC; color:#64748B; font-weight:800; text-align:left; border-bottom:1px solid #E2E8F0; padding:9px 10px; z-index:2;}
.hover-table td {border-bottom:1px solid #E2E8F0; padding:9px 10px; color:#0F172A; vertical-align:top; white-space:nowrap;}
.hover-table tr:hover {background:#EFF6FF; outline:2px solid #BFDBFE; cursor:pointer;}
.hover-table .tooltip-anchor {position:relative; font-weight:800; color:#1D4ED8;}
.hover-table tr:hover .tooltip-anchor::after {content:attr(data-tooltip); position:fixed; left:32px; top:138px; width:min(760px, 88vw); max-height:62vh; overflow:auto; z-index:999999; white-space:pre-line; text-align:left; background:#0F172A; color:#FFFFFF; border:1px solid #334155; border-radius:16px; padding:16px 18px; box-shadow:0 18px 46px rgba(15,23,42,.35); font-weight:500; line-height:1.45;}
...
st.markdown(
    ...
    <div class="hero">
      ...
      <div class="hero-pill">MVP V10.5 • Tooltip de chamados • Visual profissional</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
def _chamado_hover_table(df: pd.DataFrame, visible_cols: list[str], max_rows: int = 80) -> str:
    """Tabela HTML com tooltip por linha para visualizar todos os campos do chamado sem rolagem horizontal."""
    ...
    for _, row in view.iterrows():
        ...
        for col in view.columns:
            ...
            if val != "":
                detalhe.append(f"{col}: {val}")
        tooltip = html.escape("\n".join(detalhe), quote=True)
        cells = []
        for i, col in enumerate(visible_cols):
            val = html.escape(_format_hover_value(row.get(col, "")))
            if i == 0:
                cells.append(f'<td class="tooltip-anchor" data-tooltip="{tooltip}">{val}</td>')
            else:
    ...
    return f"""
    <div class="hover-hint">Passe o mouse sobre qualquer linha para abrir uma caixa com todos os campos do chamado.</div>
    {aviso}
...
def _parametros_exportacao(nome_arquivo: str, somente_salvos: bool, incluir_pendencias: bool, template: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame([
        {"Parâmetro": "Versão do aplicativo", "Valor": "MVP V10.5"},
