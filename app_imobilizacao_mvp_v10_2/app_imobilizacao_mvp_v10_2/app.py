...
st.title("Capitalização de Ativos - Imobilizações Adições")
st.caption("MVP V10.2: seleção inteligente CJI3 + gestão de chamados + lote acumulado + exportação AS01 permanente.")
...
def _parametros_exportacao(nome_arquivo: str, somente_salvos: bool, incluir_pendencias: bool, template: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame([
        {"Parâmetro": "Versão do aplicativo", "Valor": "MVP V10.2"},
...
with tab_chamados:
    ...
    if chamados.empty:
        ...
        st.download_button(
            ...
            file_name="Gestao_Chamados_MVP_V10_2.xlsx",
...
with tab_template:
    ...
    with p1:
        nome_arquivo = st.text_input("Nome do arquivo Excel", value="Template_AS01_MVP_V10_2.xlsx")
    with p2:
