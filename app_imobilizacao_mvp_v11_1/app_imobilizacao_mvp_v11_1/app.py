...
.selection-toolbar {border:1px solid #CBD5E1; border-radius:16px; padding:12px 14px; background:linear-gradient(90deg,#FFFFFF,#F8FAFC); box-shadow:0 6px 18px rgba(15,23,42,.055); margin:8px 0 12px 0;}
.selection-toolbar-title {font-size:.78rem; color:#2563EB; font-weight:900; letter-spacing:.08em; text-transform:uppercase; margin-bottom:4px;}
.selection-toolbar-note {font-size:.82rem; color:#64748B;}
.selection-grid-title {display:flex; align-items:center; justify-content:space-between; gap:12px; margin-top:12px;}
.selection-grid-title b {font-size:1rem; color:#0F172A;}
...
st.markdown(
    ...
    <div class="hero">
      ...
      <div class="hero-pill">MVP V11.1 • Relatório SAP S_ALR_87011964 integrado • Visual profissional</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
def _peps_from_saved_decisions(decisoes: list[dict], cji3_df: pd.DataFrame | None = None) -> set[str]:
    """Identifica os PEPs que já possuem decisão salva/cadastrada no ativo.

    A regra usa duas fontes para maior segurança:
    1) o campo Fabricante / Pep salvo na decisão;
    2) os ID_CJI3 de origem, cruzados com a CJI3 enriquecida para recuperar o PEP.
    """
    peps: set[str] = set()
    for d in decisoes or []:
        # Considera somente decisões efetivamente salvas.
        status = _str_clean(d.get("Status", "Salvo"))
        if status and status.lower() not in {"salvo", "cadastrado", "cadastrado no ativo"}:
            continue
        pep_dec = _str_clean(d.get("Fabricante / Pep", ""))
        if pep_dec:
            for part in pep_dec.replace(";", ",").replace("|", ",").split(","):
                part = part.strip()
                if part:
                    peps.add(part)

    ids = _ids_from_saved_decisions(decisoes)
    if ids and cji3_df is not None and isinstance(cji3_df, pd.DataFrame) and not cji3_df.empty:
        if "ID_CJI3" in cji3_df.columns and "PEP" in cji3_df.columns:
            base = cji3_df.copy()
            base["ID_CJI3"] = pd.to_numeric(base["ID_CJI3"], errors="coerce")
            peps.update(
                base[base["ID_CJI3"].isin(ids)]["PEP"]
                .dropna()
                .astype(str)
                .str.strip()
                .loc[lambda x: x != ""]
...
def _parametros_exportacao(nome_arquivo: str, somente_salvos: bool, incluir_pendencias: bool, template: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame([
        {"Parâmetro": "Versão do aplicativo", "Valor": "MVP V11.1"},
...
def _find_col(df: pd.DataFrame, candidates: list[str]) -> str | None:
    """Localiza uma coluna por nomes prováveis, sem depender de um layout único do relatório SAP."""
    if df is None or df.empty:
        return None
    norm_map = {norm_text(c).replace(" ", ""): c for c in df.columns}
    for cand in candidates:
        key = norm_text(cand).replace(" ", "")
        if key in norm_map:
            return norm_map[key]
    # fallback por contém
    for cand in candidates:
...
def _ativos_profile(df: pd.DataFrame) -> dict:
    """Mapeia o modelo SAP S_ALR_87011964 e variações do relatório de ativos."""
    return {
        "empresa": _find_col(df, ["Empresa", "Sociedade", "Código Empresa"]),
        "ativo": _find_col(df, ["Ativo", "N IMOB", "Nº Ativo", "Número do ativo", "Imobilizado", "Imob"]),
        "subnumero": _find_col(df, ["Subnúmero", "Subnumero", "Sub número", "Sub nº", "Sub ativo"]),
        "classe": _find_col(df, ["Classe", "Classe do ativo", "Classe Ativo", "Classe imobilizado"]),
        "cap_custo": _find_col(df, ["CAP Custo", "Conta custo", "Conta de custo", "Conta APC"]),
        "cap_deprec": _find_col(df, ["CAP Deprec", "Conta depreciação", "Conta depreciacao", "Deprec"]),
        "descricao": _find_col(df, ["Descrição do Ativo", "Denominação", "Descrição", "Descr_01", "Texto", "Denominação do ativo"]),
        "centro": _find_col(df, ["Centro", "Centro custo", "Centro de custo", "C.C SAP", "CC SAP"]),
        "centro_custo": _find_col(df, ["Centro de Custo", "Centro custo", "CC SAP", "C.C SAP"]),
        "centro_lucro": _find_col(df, ["Centro de Lucro", "Centro lucro"]),
        "local_negocios": _find_col(df, ["Local de Negócios", "Local de Negocios", "Local"]),
        "pep": _find_col(df, ["PEP", "Elemento PEP", "Projeto"]),
        "fornecedor": _find_col(df, ["Fornecedor", "Código Fornecedor", "Cod Fornecedor"]),
        "pedido": _find_col(df, ["Pedido Compra", "Pedido", "Pedido de Compra"]),
        "documento_fi": _find_col(df, ["Documento FI", "Documento", "Doc FI"]),
        "data_cap": _find_col(df, ["Data Capitalização", "Data Capitalizacao", "Data cap"]),
        "data_aquis": _find_col(df, ["Data Aquisição", "Data Aquisicao", "Data aquisição"]),
        "valor_aquis": _find_col(df, ["Valor Aquisição", "Valor Aquisicao", "Valor de aquisição", "APC", "Valor"]),
        "deprec_acum": _find_col(df, ["Depreciação Acumulada", "Depreciacao Acumulada", "Deprec acumulada"]),
        "deprec_exercicio": _find_col(df, ["Depreciação do Exercício", "Depreciacao do Exercicio", "Deprec exercício"]),
        "valor_liquido": _find_col(df, ["Valor Líquido", "Valor Liquido", "Valor contábil líquido", "Valor contábil"]),
        "vida_util": _find_col(df, ["Vida Útil Meses", "Vida Util Meses", "Vida útil", "Vida Util"]),
        "moeda": _find_col(df, ["Moeda"]),
        "tag": _find_col(df, ["Tag Patrimonial", "TAG", "Plaqueta"]),
        "status": _find_col(df, ["Status", "Situação", "Situacao"]),
        "serie": _find_col(df, ["Número Série", "Numero Serie", "Série", "Serie"]),
        "inventario": _find_col(df, ["Inventário", "Inventario"]),
        # campos genéricos usados pelos filtros antigos
        "conta": _find_col(df, ["CAP Custo", "Conta", "Conta SAP", "Conta contábil", "Conta Razão", "Conta do Razão"]),
        "valor": _find_col(df, ["Valor Líquido", "Valor Liquido", "Valor Aquisição", "Valor Aquisicao", "Valor contábil", "APC", "Valor"]),
    }
...
def _to_num_series(s: pd.Series) -> pd.Series:
    ...
    # Trata tanto padrão brasileiro (1.234,56) quanto padrão numérico do SAP exportado (1234.56).
    has_comma = raw.str.contains(",", regex=False)
    br = raw.str.replace(".", "", regex=False).str.replace(",", ".", regex=False)
    normalized = raw.where(~has_comma, br)
    return pd.to_numeric(normalized, errors="coerce").fillna(0)

def _excel_serial_to_date_display(s: pd.Series) -> pd.Series:
    nums = pd.to_numeric(s, errors="coerce")
    converted = pd.to_datetime(nums, unit="D", origin="1899-12-30", errors="coerce")
    out = s.astype(str).replace("nan", "")
    mask = nums.notna() & nums.between(20000, 70000)
    out.loc[mask] = converted.loc[mask].dt.strftime("%d/%m/%Y")
    return out

def _prepare_ativos_sap_model(df: pd.DataFrame) -> pd.DataFrame:
    """Ajustes específicos para o modelo S_ALR_87011964 carregado pelo usuário."""
    if df is None or df.empty:
        return pd.DataFrame()
    out = df.copy()
    out.columns = [str(c).strip() for c in out.columns]
    perfil = _ativos_profile(out)
    for key in ["data_cap", "data_aquis"]:
        col = perfil.get(key)
        if col and col in out.columns:
            out[col] = _excel_serial_to_date_display(out[col])
    for key in ["ativo", "subnumero", "classe", "cap_custo", "cap_deprec", "centro", "centro_custo", "centro_lucro", "local_negocios", "pep", "fornecedor", "pedido", "documento_fi", "tag", "serie", "inventario", "empresa"]:
        col = perfil.get(key)
        if col and col in out.columns:
            out[col] = out[col].astype(str).replace("nan", "").str.replace(".0", "", regex=False).str.strip()
    return out


tab_upload, tab_chamados, tab_cadastro, tab_template, tab_ativos = st.tabs(
    ["1. Carregamento das Bases", "2. Gestão de Chamados", "3. Cadastro / Decisão", "4. Template AS01 / Exportação", "5. Ativos Existentes"]
)
...
with tab_upload:
    ...
    with c4:
        arq_ativos = st.file_uploader("Relatório de Ativos Existentes - SAP S_ALR_87011964", type=["xlsx", "xlsm"], key="up_ativos")
    ...
    if st.button("Processar bases carregadas", type="primary"):
        ...
        try:
            ...
            if arq_ativos:
                ativos_df = read_excel_any(arq_ativos)
                ativos_df = _prepare_ativos_sap_model(ativos_df)
                bases["ativos_existentes"] = ativos_df
...
with tab_chamados:
    ...
    if chamados.empty:
        ...
        # Integração com a aba 3: quando um lote é salvo, todos os PEPs ligados aos IDs_CJI3 tratados
        # passam a ser marcados aqui como cadastrados no ativo.
        peps_cadastrados_ativo = _peps_from_saved_decisions(st.session_state.decisoes, cji3_base)
        if "PEP" in ch_view.columns:
            ch_view["Status Ativo"] = ch_view["PEP"].astype(str).str.strip().apply(
                lambda x: "✅ Cadastrado no Ativo" if x in peps_cadastrados_ativo else "⏳ Pendente de Cadastro"
            )
        else:
            ch_view["Status Ativo"] = "⏳ Pendente de Cadastro"
        ...
        m5.metric("Cadastrados Ativo", int(ch_view["Status Ativo"].astype(str).str.contains("Cadastrado", na=False).sum()) if "Status Ativo" in ch_view else 0)
        m6.metric("Etapas", ch_view["ETAPA SOLICITAÇÃO"].astype(str).nunique() if "ETAPA SOLICITAÇÃO" in ch_view else 0)
        m7.metric("Filiais", ch_view["FILIAL"].astype(str).nunique() if "FILIAL" in ch_view else 0)
        ...
        st.markdown('<div class="prof-divider"></div>', unsafe_allow_html=True)
        col_prioridade = [
            "N_CHAMADO", "Status Ativo", "ETAPA SOLICITAÇÃO", "CLASSE DO CHAMADO", "FILIAL", "Empresa", "Centro", "PEP",
        ...
        if pep_detalhe:
            ...
            with d1:
                ...
                cols_visiveis_tooltip = [c for c in ["N_CHAMADO", "Status Ativo", "ETAPA SOLICITAÇÃO", "CLASSE DO CHAMADO", "FILIAL", "Empresa", "Centro", "PEP", "Inventário", "Série", "NF Chamado", "Pedido Chamado", "Existe na CJI3"] if c in detalhe_ch.columns]
        ...
        pacote_chamados = excel_bytes({
            ...
            "PEPs_Cadastrados_Ativo": pd.DataFrame({"PEP_Cadastrado_Ativo": sorted(peps_cadastrados_ativo)}),
            "Chamados_Original": chamados,
        })
        st.download_button(
            "Baixar gestão de chamados em Excel",
            data=pacote_chamados,
            file_name="Gestao_Chamados_MVP_V11.xlsx",
...
with tab_template:
    ...
    with p1:
        nome_arquivo = st.text_input("Nome do arquivo Excel", value="Template_AS01_MVP_V11.xlsx")
    with p2:
...
with tab_ativos:
    st.markdown('<div class="section-label">Consulta e análise dos ativos já existentes</div>', unsafe_allow_html=True)
    st.subheader("Ativos Existentes - SAP S_ALR_87011964")
    ativos = st.session_state.bases.get("ativos_existentes", pd.DataFrame()).copy()

    if ativos.empty:
        st.warning("Carregue o relatório de ativos existentes SAP S_ALR_87011964 na aba 1 para habilitar esta consulta.")
        st.info("Modelo esperado: Empresa, Ativo, Subnúmero, Classe, CAP Custo, CAP Deprec, Descrição do Ativo, Centro, Centro de Custo, Centro de Lucro, Local de Negócios, PEP, Fornecedor, Pedido Compra, Documento FI, datas e valores.")
    else:
        ativos = _prepare_ativos_sap_model(ativos)
        for col in ativos.columns:
            if ativos[col].dtype == "object":
                ativos[col] = ativos[col].astype(str).replace("nan", "")

        perfil = _ativos_profile(ativos)
        col_ativo = perfil["ativo"]
        col_sub = perfil["subnumero"]
        col_classe = perfil["classe"]
        col_conta = perfil["conta"]
        col_centro = perfil["centro"]
        col_desc = perfil["descricao"]
        col_valor_aquis = perfil["valor_aquis"]
        col_valor_liq = perfil["valor_liquido"]
        col_empresa = perfil["empresa"]
        col_status = perfil["status"]
        col_pep = perfil["pep"]

        st.markdown(
            '<div class="selection-help"><b>Objetivo desta aba</b><br>'
            'Consultar a relação SAP S_ALR_87011964 para identificar ativos existentes, componentes, subnúmeros e possíveis complementos antes de decidir se o lançamento CJI3 será novo ativo, componente ou complemento.</div>',
            unsafe_allow_html=True,
        )

        f1, f2, f3, f4, f5, f6 = st.columns([1, 1, 1, 1, 1, 1.6])
        with f1:
            filtro_ativo = st.text_input("Número do ativo", placeholder="Ex.: 180000004", key="flt_ativo_existente")
        with f2:
            classe_opts = _unique_options(ativos, col_classe) if col_classe else [""]
            filtro_classe = st.selectbox("Classe", classe_opts, key="flt_classe_ativo_existente")
        with f3:
            conta_opts = _unique_options(ativos, col_conta) if col_conta else [""]
            filtro_conta = st.selectbox("Conta / CAP Custo", conta_opts, key="flt_conta_ativo_existente")
        with f4:
            centro_opts = _unique_options(ativos, col_centro) if col_centro else [""]
            filtro_centro = st.selectbox("Centro", centro_opts, key="flt_centro_ativo_existente")
        with f5:
            status_opts = _unique_options(ativos, col_status) if col_status else [""]
            filtro_status = st.selectbox("Status", status_opts, key="flt_status_ativo_existente")
        with f6:
            filtro_desc = st.text_input("Descrição / PEP / Fornecedor", placeholder="Buscar por descrição, PEP, fornecedor, tag...", key="flt_desc_ativo_existente")

        view = ativos.copy()
        if filtro_ativo and col_ativo:
            view = view[view[col_ativo].astype(str).str.contains(filtro_ativo, case=False, na=False)]
        if filtro_classe and col_classe:
            view = view[view[col_classe].astype(str).str.strip() == filtro_classe]
        if filtro_conta and col_conta:
            view = view[view[col_conta].astype(str).str.strip() == filtro_conta]
        if filtro_centro and col_centro:
            view = view[view[col_centro].astype(str).str.strip() == filtro_centro]
        if filtro_status and col_status:
            view = view[view[col_status].astype(str).str.strip() == filtro_status]
        if filtro_desc:
            nt = norm_text(filtro_desc)
            mask = view.astype(str).apply(lambda col: col.map(norm_text).str.contains(nt, na=False)).any(axis=1)
            view = view[mask]

        valor_aquis_total = _to_num_series(view[col_valor_aquis]).sum() if col_valor_aquis and col_valor_aquis in view.columns else 0.0
        valor_liq_total = _to_num_series(view[col_valor_liq]).sum() if col_valor_liq and col_valor_liq in view.columns else 0.0
        c1, c2, c3, c4, c5, c6 = st.columns(6)
        c1.metric("Qtde de Registros", len(view))
        c2.metric("Ativos distintos", view[col_ativo].astype(str).nunique() if col_ativo else 0)
        c3.metric("Subnúmeros", view[col_sub].astype(str).nunique() if col_sub else 0)
        c4.metric("Classes", view[col_classe].astype(str).nunique() if col_classe else 0)
        c5.metric("Valor aquisição", f"{valor_aquis_total:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
        c6.metric("Valor líquido", f"{valor_liq_total:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))

        st.markdown('<div class="prof-divider"></div>', unsafe_allow_html=True)
        st.markdown('<div class="section-label">Valores por classe</div>', unsafe_allow_html=True)
        if col_classe:
            rank_classe = view.copy()
            rank_classe["Valor Aquisição"] = _to_num_series(rank_classe[col_valor_aquis]) if col_valor_aquis else 0
            rank_classe["Valor Líquido"] = _to_num_series(rank_classe[col_valor_liq]) if col_valor_liq else 0
            rank_classe = rank_classe.groupby(col_classe, dropna=False).agg(
                Qtde=(col_classe, "size"),
                Ativos=(col_ativo, "nunique") if col_ativo else (col_classe, "size"),
                Valor_Aquisicao=("Valor Aquisição", "sum"),
                Valor_Liquido=("Valor Líquido", "sum"),
            ).reset_index().sort_values("Valor_Aquisicao", ascending=False)
            rank_classe["% Valor Aquisição"] = (rank_classe["Valor_Aquisicao"] / max(rank_classe["Valor_Aquisicao"].sum(), 1) * 100).round(2)
        else:
            rank_classe = pd.DataFrame(columns=["Classe", "Qtde", "Ativos", "Valor_Aquisicao", "Valor_Liquido", "% Valor Aquisição"])

        g1, g2 = st.columns([1.2, 1.5])
        with g1:
            st.markdown('<div class="asset-card"><b>Resumo por Classe</b>', unsafe_allow_html=True)
            st.dataframe(rank_classe, use_container_width=True, height=320, hide_index=True)
            st.markdown('</div>', unsafe_allow_html=True)
        with g2:
            st.markdown('<div class="asset-card"><b>Valor de Aquisição por Classe</b>', unsafe_allow_html=True)
            if not rank_classe.empty and "Valor_Aquisicao" in rank_classe.columns:
                chart_df = rank_classe.copy()
                chart_df.iloc[:, 0] = chart_df.iloc[:, 0].astype(str)
                st.bar_chart(chart_df.set_index(chart_df.columns[0])["Valor_Aquisicao"])
            else:
                st.info("Não foi possível identificar Classe e Valor Aquisição para montar o gráfico.")
            st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="prof-divider"></div>', unsafe_allow_html=True)
        st.markdown('<div class="section-label">Base analítica de ativos existentes</div>', unsafe_allow_html=True)
        prioridade = [
            perfil.get("empresa"), perfil.get("ativo"), perfil.get("subnumero"), perfil.get("classe"), perfil.get("cap_custo"), perfil.get("cap_deprec"),
            perfil.get("descricao"), perfil.get("centro"), perfil.get("centro_custo"), perfil.get("centro_lucro"), perfil.get("local_negocios"), perfil.get("pep"),
            perfil.get("fornecedor"), perfil.get("pedido"), perfil.get("documento_fi"), perfil.get("data_cap"), perfil.get("data_aquis"),
            perfil.get("valor_aquis"), perfil.get("deprec_acum"), perfil.get("deprec_exercicio"), perfil.get("valor_liquido"), perfil.get("vida_util"),
            perfil.get("moeda"), perfil.get("tag"), perfil.get("status"), perfil.get("serie"), perfil.get("inventario"),
        ]
        prioridade = [c for c in prioridade if c and c in view.columns]
        demais = [c for c in view.columns if c not in prioridade]
        cols_view = prioridade + demais
        st.caption("Modelo adequado ao layout SAP S_ALR_87011964. Use os filtros acima para localizar ativo principal, subnúmero, conta, centro, PEP ou descrição.")
        st.dataframe(view[cols_view], use_container_width=True, height=460, hide_index=True)

        pacote_ativos = excel_bytes({
            "Ativos_Filtrados": view[cols_view],
            "Valores_por_Classe": rank_classe,
            "Ativos_Original": ativos,
        })
        st.download_button(
            "Baixar consulta de ativos em Excel",
            data=pacote_ativos,
            file_name="Consulta_Ativos_Existentes_V11_1.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )
...
with tab_cadastro:
    ...
    if df.empty:
        ...
        st.markdown(
            '<div class="selection-grid-title"><b>Quadro CJI3 para seleção</b>'
            '<span class="compact-note">Selecione os itens diretamente na grade. A marcação por cor já está integrada nesta visualização.</span></div>',
            unsafe_allow_html=True,
        )
        st.markdown('<div class="selection-toolbar"><div class="selection-toolbar-title">Ferramentas de seleção rápida</div>', unsafe_allow_html=True)
        vlim1, vlim2, vlim3, vlim4 = st.columns([1, 1.2, 1.2, 3])
        with vlim1:
        ...
        with vlim2:
            selecionar_todos_visiveis = st.checkbox("Selecionar todos visíveis", value=False, help="Marca todos os itens disponíveis exibidos nesta grade, respeitando os filtros aplicados.")
        with vlim3:
            manter_lote_marcado = st.checkbox("Manter lote marcado", value=True, help="Mantém marcado o que já está acumulado no lote atual.")
        with vlim4:
            st.markdown('<div class="selection-toolbar-note">Use os filtros acima para reduzir a base e marque todos os registros visíveis de uma só vez quando todo o conjunto receber o mesmo tratamento.</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        grid_exibicao = grid.head(int(limite_linhas_grade)).copy()
        def _marcar_inicial(r):
            try:
                id_linha = int(r["ID_CJI3"])
            except Exception:
                return False
            if r.get("Situação") == "Salvo / bloqueado":
                return False
            if selecionar_todos_visiveis and r.get("Situação") == "Disponível":
                return True
            return manter_lote_marcado and id_linha in ids_atuais
        grid_exibicao.insert(0, "Selecionar", grid_exibicao.apply(_marcar_inicial, axis=1))

        edited = st.data_editor(
            grid_exibicao.style.apply(_style_status_rows, axis=1),
            use_container_width=True,
            height=440,
            hide_index=True,
            disabled=[c for c in grid_exibicao.columns if c != "Selecionar"],
            column_config={
