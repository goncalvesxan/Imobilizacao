...
st.set_page_config(page_title="Motor de Imobilização SAP - MVP V10", layout="wide")
...
st.caption("MVP V10: seleção inteligente CJI3 + gestão de chamados + lote acumulado + exportação AS01 padronizada.")
...
def _parametros_exportacao(nome_arquivo: str, somente_salvos: bool, incluir_pendencias: bool, template: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame([
        {"Parâmetro": "Versão do aplicativo", "Valor": "MVP V10"},
...
with tab_template:
    ...
    with p1:
        nome_arquivo = st.text_input("Nome do arquivo Excel", value="Template_AS01_MVP_V10.xlsx")
    with p2:
    ...
    if not st.session_state.decisoes:
        st.warning("Nenhum item salvo na Central de Decisão. Salve ao menos um lote para gerar o Template AS01.")
        ...
        st.caption("Esta é a visão principal para conferência antes de enviar ao SAP. A rastreabilidade fica nas demais abas do pacote Excel.")
